...
def main():
    with db.session_scope() as ses:
        ...
        for lc in query.all():
            ...
            if "vs_type" in feats.columns:
                print "skiping"
                continue

            srcs = pd.DataFrame(tile.load_npy_file()[["id", "vs_type", "ra_k", "dec_k"]])
            srcs = srcs[srcs.id.isin(feats.id)]

            new_feats = pd.merge(srcs, feats, on="id", how="inner").drop("ogle3_type", axis=1)
